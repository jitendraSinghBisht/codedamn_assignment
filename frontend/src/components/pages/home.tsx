...
export function Home() {
  ...
  async function getFiles(volumeName: string){
    const response = await fetch(
      `${process.env.URL}/api/container/get-root-structure`,
      {
        method: "POST",
        mode: "no-cors",
        headers:{
  ...
  async function deploy(){
    ...
    const response = await fetch(
      `${process.env.URL}/api/container/create`,
      {
        method: "POST",
        mode: "no-cors",
        headers:{
  ...
  async function getProject(){ 
    ...
    if (!old && !oldVolumes.length) {
      const response = await fetch(`${process.env.URL}/api/container/get-old-volumes`,{mode: "no-cors"});
      const jres: IApiResponse | IApiError = await response.json();
      if (jres.statusCode >= 400) {
